...
async function fetchAndSave(config, fetchedEntries = []) {
  ...
  return fs.writeFile(outputFile, JSON.stringify(fetchedEntries, null, 2), { flag: 'w' }); // eslint-disable-line security/detect-non-literal-fs-filename
}
...
async function processAlumniPage(page) {
  ...
  tableRows.forEach((tableRowNode) => {
    ...
    alumnis.push({
      ...
      date_left_program: tableRowCellsNodes[3].dataset.time,
