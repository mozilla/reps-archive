...
async function fetchAndSave(config, fetchedEntries = []) {
  ...
  const lastEntry = fetchedEntries[fetchedEntries.length - 1]; // eslint-disable-line no-magic-numbers
  const needsMore = config.since ? lastEntry && new Date(lastEntry[config.dateField]) > config.since : true;

  if (data.next && needsMore) {
    return fetchAndSave({
      url: data.next,
      name,
      since: config.since,
  ...
  let dedupedEntries = fetchedEntries;

  const outputFile = path.join(__dirname, DATA_PATH, `${name}.json`);
  if (config.since) {
    debug('only fetched entries since', config.since);
    const existingEntriesContent = await fs.readFile(outputFile, 'utf-8');
    const existingEntries = JSON.parse(existingEntriesContent);
    const dedupedNewEntries = fetchedEntries.filter((entry) => !existingEntries.find((existingEntry) => existingEntry.remo_url === entry.remo_url));
    if (!dedupedNewEntries.length) {
      debug('No new entries found, saving nothing');
      return;
    }
    dedupedEntries = dedupedNewEntries
      .concat(existingEntries)
      .sort((a, b) => b[config.dateField] - a[config.dateField]);
  }

  debug(`Writing data to ${outputFile}`);
  return fs.writeFile(outputFile, JSON.stringify(dedupedEntries, null, JSON_SPACES), { flag: 'w' }); // eslint-disable-line security/detect-non-literal-fs-filename
}
